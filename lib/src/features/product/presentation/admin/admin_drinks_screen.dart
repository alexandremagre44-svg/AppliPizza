...
class _AdminDrinksScreenState extends State<AdminDrinksScreen> {
  ...
  Future<void> _loadDrinks() async {
    ...
    final localDrinks = await _crudRepository.loadDrinks();
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          if (formKey.currentState!.validate()) {
                            ...
                            final firestoreSuccess = await _firestoreRepository.saveDrink(newDrink);
                            
                            // Sauvegarder aussi en local pour backup
                            if (isNew) {
                              success = await _crudRepository.addDrink(newDrink);
                            } else {
                              success = await _crudRepository.updateDrink(newDrink);
                            }
  ...
  Future<void> _deleteDrink(Product drink) async {
    ...
    if (confirm == true) {
      ...
      final firestoreSuccess = await _firestoreRepository.deleteDrink(drink.id);
      
      // Supprimer aussi du local
      final localSuccess = await _crudRepository.deleteDrink(drink.id);
