...
class _NewPageDialogState extends State<NewPageDialog> {
  ...
  void _onTitleChanged() {
    // Auto-generate page ID from title if user hasn't manually edited it
    if (_pageIdController.text.isEmpty || 
        _pageIdController.text == _generatePageId(_titleController.text.substring(0, _titleController.text.length > 1 ? _titleController.text.length - 1 : 0))) {
      _pageIdController.text = _generatePageId(_titleController.text);
    }
  }
